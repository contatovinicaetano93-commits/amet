...
export default async function ClassManagePage({ params }: PageProps) {
  ...
  try {
    ...
    console.error("[ava-turma-gerir]", error);
    notFound();
  }

  if (!row) notFound();

  if (!canManageClass(session.user.role, row.teacherId, session.user.id)) {
    redirect("/ava");
  }
